...
class ConvNet(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 10, 5) # 输入通道数1，输出通道数10，核的大小5,输出通道数由核的数量决定
...
def train(model, device, train_loader, optimizer, epoch):
    model.train()
    for batch_idx, (data, target) in enumerate(train_loader):#遍历数据对象，同时列出数据以及下标：每一步释放512个图片学习
        data, target = data.to(device), target.to(device)#取出mini-batch的数据和标签
        optimizer.zero_grad()#梯度归零
        output = model(data)
        loss = F.nll_loss(output, target) # 计算神经网络的预测值与实际的误差
...
def test(model, device, test_loader):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            correct += pred.eq(target.view_as(pred)).sum().item()#正确的个数
